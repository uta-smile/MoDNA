...
class ElectraForPreTrainingOutput(ModelOutput):
    ...
    motif_logits: torch.FloatTensor = None
...
class ElectraModel(ElectraPreTrainedModel):
    def __init__(self, config,ltr=False,rtl=False):
    ...
    def forward(
        ...
        #if motif_label is not None:
        #    motif_shape = motif_label.size()
        ...
        if token_type_ids is None:
           ...
           # motif_token_type_ids = torch.zeros(motif_shape, dtype=torch.float, device=device)

        extended_attention_mask = self.get_extended_attention_mask(attention_mask, input_shape, device)

        if self.ltr or self.rtl:
            causal_mask = torch.ones(input_shape, device=device)
            if ltr:
                causal_mask = torch.tril(causal_mask,diagonal=-1)
            else:
                causal_mask = torch.tril(causal_mask,diagonal=0)
            extended_attention_mask*=causal_mask.unsqueeze(0)
...
class ElectraForPreTraining(ElectraPreTrainedModel):
    def __init__(self, config):
        ...
        self.motif_prediction = ElectraDiscriminatorPredictions(config)
        self.init_weights()
    ...
    def forward(
        ...
        motif_logits = self.motif_prediction(discriminator_sequence_output)
...
class ElectraForMaskedLM(ElectraPreTrainedModel):
    def __init__(self, config,args, ltr=False,rtl=False):
        super().__init__(config)

        self.electra = ElectraModel(config)
        self.generator_predictions = ElectraGeneratorPredictions(config)

        #self.sequence_output = ElectraGeneratorOurput(config)
        self.generator_lm_head = nn.Linear(config.embedding_size, config.vocab_size)
        self.motif_prediction = nn.Linear(config.embedding_size, 24)
        self.init_weights()
        self.args = args
    ...
    def get_generator_predictions(self):
       
        return self.generator_predictions
    ...
    def forward(
        ...
        **kwargs
        ...
        self.sequence_output = generator_sequence_output.detach()
        prediction_scores_ori = self.generator_predictions(generator_sequence_output)

        
        prediction_scores = self.generator_lm_head(prediction_scores_ori)

       

        
        prediction_scores_motif = self.motif_prediction(prediction_scores_ori)
        ...
        L = MaskedLMOutput(
            ...
            logits_motif = prediction_scores_motif,
            hidden_states=generator_hidden_states.hidden_states,
            attentions=generator_hidden_states.attentions,
        )

        
        Motif_L = MaskedLMOutput(
            loss=motif_loss,
            logits=prediction_scores_motif,
            hidden_states=generator_hidden_states.hidden_states,
            attentions=generator_hidden_states.attentions,
        )
        ...
        return [L]
        #return pack_prediction

      #  if self.args.model == "ELECTRA":

            
       
     #   elif self.args.model == "ELECTRIC":

     #       return generator_sequence_output
...
class TwoTowerClozeTransformer(ElectraForMaskedLM):
    def __init__(self, config,args):
        super().__init__(config,args)
        self.ltr = ElectraForMaskedLM(config,args, ltr=True)
        self.rtl = ElectraForMaskedLM(config,args,  rtl=True)
    ...
    def forward(self,
        ...
        **kwargs):
        ...
        self._sequence_output = torch.cat([roll(ltr_reprs, -1),
                                       roll(rtl_reprs, 1)], -1)

        
        self._embedding_table = self.ltr.electra.get_input_embeddings()
        #self._embedding_table = self.ltr.embedding_table
